...
interface Message { id: string; sender: 'user' | 'assistant'; content: string; timestamp: string; executionId?: string; provenance?: unknown[]; status?: string; retrievalUsed?: boolean; memoryUsed?: boolean; attachments?: Pick<Attachment, 'name' | 'mime_type' | 'kind'>[]; telemetry?: { contextWindow?: number; promptTokens?: number; outputTokens?: number; totalTokens?: number; promptTps?: number; generationTps?: number; memoryTokens?: number; knowledgeTokens?: number }; requestId?: string; }
...
interface ChatOptions { contextWindow: number; }
const DEFAULT_OPTIONS: ChatOptions = { contextWindow: 8192 };
...
export const ChatTab: React.FC = () => {
  ...
  const handleSendMessage = async (event: React.FormEvent) => {
    ...
    setMessages((previous) => [...previous, userMsg]); setInputMessage(''); setAttachments([]); setLoading(true); setError('');
    try {
      const response = await fetch('/api/v1/chat', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-request-id': requestId }, body: JSON.stringify({ conversation_id: conversationId, message: userText, conversation_history: history, attachments: outgoing, runtime_options: { context_window: options.contextWindow } }) });
