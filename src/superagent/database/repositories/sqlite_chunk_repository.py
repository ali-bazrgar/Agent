...
class SqliteChunkRepository(ChunkRepository):
    ...
    def create_chunk(self, chunk: DocumentChunk) -> DocumentChunk:
        try:
            with self.engine.connect() as connection:
                connection.execute(
                    ...
                    (
                        ...
                        chunk.chunk_index,
                        chunk.token_count,
                        self.engine.to_json(chunk.metadata),
                ...
                try:
                    connection.execute(
                        """
                        INSERT INTO knowledge_chunks (chunk_id, document_id, version_id, content, content_hash, chunk_index, token_count, character_count, language, metadata_json, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """,
                        (
                            chunk.chunk_id,
                            ...
                            chunk.chunk_index,
                            chunk.token_count,
                            chunk.character_count or len(chunk.content),
                            chunk.language,
                            self.engine.to_json(chunk.metadata),
                            chunk.created_at.isoformat(),
                        ),
            ...
            raise PersistenceError(f"failed to create chunk: {exc}") from exc
        return chunk
