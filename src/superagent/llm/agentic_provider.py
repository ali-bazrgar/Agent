...
class AgenticLLMProvider(LLMProvider):
    ...
    def _effective_capabilities(self) -> ProviderCapabilities:
        ...
        if not model_id:
            # Without a concrete model identity there is no safe way to apply
            # model-level verification. Non-strict deployments may retain the
            # provider declaration; strict deployments fail closed for tools.
            if getattr(self.settings, "require_verified_capabilities", False):
                effective = provider.model_copy(update={"tool_calling": False, "structured_output": False})
                return effective
            return provider

        policy = CapabilityPolicy(
            self._capability_registry,
            require_verified=getattr(self.settings, "require_verified_capabilities", False),
    ...
    def complete(self, request: LLMRequest) -> LLMResponse:
        ...
        if not effective.tool_calling or not getattr(self.settings, "llm_driven_tools", True):
