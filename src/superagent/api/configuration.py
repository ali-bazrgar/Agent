...
def get_configuration(container: AppContainer = Depends(get_container)) -> dict[str, object]:
    ...
    return {
        ...
        "embeddings": {
            **_provider(settings.embedding_base_url, settings.embedding_model_id, "llama.cpp"),
            "capabilities": _capabilities(container.embedding_provider),
        },
        "reranker": {
            **_provider(settings.reranker_base_url, settings.reranker_model_id, "llama.cpp"),
            "capabilities": _capabilities(container.reranker_provider),
        },
        "agent": {
            ...
            "llmDrivenTools": settings.llm_driven_tools,
            "llmDrivenMemory": settings.llm_driven_memory,
            "automaticMemoryExtractionEnabled": settings.automatic_memory_extraction_enabled,
        },
...
def get_model_runtime_catalog(container: AppContainer = Depends(get_container)) -> dict[str, Any]:
    ...
    return {
        "llm": {
            ...
            "runtime": container.runtime_config.model_dump(mode="json") if container.runtime_config else None,
            "capabilities": _capabilities(container.llm_provider),
            "controls": {
                "temperature": {"min": 0.0, "max": 2.0, "step": 0.01, "value": settings.llm_temperature},
                "topP": {"min": 0.01, "max": 1.0, "step": 0.01, "value": settings.llm_top_p},
                "frequencyPenalty": {"min": -2.0, "max": 2.0, "step": 0.01, "value": settings.llm_frequency_penalty},
                "presencePenalty": {"min": -2.0, "max": 2.0, "step": 0.01, "value": settings.llm_presence_penalty},
                "maxOutputTokens": {"min": 1, "max": container.runtime_config.max_output_tokens if container.runtime_config and container.runtime_config.max_output_tokens else settings.llm_max_output_tokens, "value": settings.llm_max_output_tokens},
                "contextWindowTokens": {"min": 256, "max": container.runtime_config.context_window_tokens if container.runtime_config else settings.context_window_tokens, "value": container.runtime_config.context_window_tokens if container.runtime_config else settings.context_window_tokens, "readOnly": True},
                "seed": {"value": settings.llm_seed, "nullable": True},
            },
        },
        "embedding": {
            "provider": "llama.cpp",
            ...
            "capabilities": _capabilities(container.embedding_provider),
            "controls": {"modelId": {"value": settings.embedding_model_id, "nullable": True}, "dimensions": {"value": None, "nullable": True}},
        },
        "reranker": {
            "provider": "llama.cpp",
            "modelId": settings.reranker_model_id,
            "baseUrl": settings.reranker_base_url,
            "capabilities": _capabilities(container.reranker_provider),
            "controls": {"modelId": {"value": settings.reranker_model_id, "nullable": True}, "topN": {"min": 1, "value": None, "nullable": True}},
        },
    }
