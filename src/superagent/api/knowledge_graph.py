...
def get_knowledge_graph(container: AppContainer = Depends(get_container)) -> dict[str, Any]:
    ...
    The graph is intentionally backed only by durable document/chunk/knowledge
    relationships. Persistence failures are converted to JSON API errors rather
    than leaking a plain-text 500 response that the frontend cannot parse.
    """
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Knowledge graph persistence is unavailable: {exc}",
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Knowledge graph could not be built: {type(exc).__name__}: {exc}",
        ) from exc
    ...
    for document in documents:
        ...
        for chunk in document.chunks:
            ...
            edges.append({
                "id": f"doc-chunk:{document.document_id}:{chunk.chunk_id}",
    ...
    for item in knowledge:
        ...
        if item.document_id:
            ...
            edges.append({
                "id": f"document-knowledge:{item.document_id}:{item.knowledge_id}",
        ...
        if item.chunk_id:
            ...
            edges.append({
                "id": f"chunk-knowledge:{item.chunk_id}:{item.knowledge_id}",
