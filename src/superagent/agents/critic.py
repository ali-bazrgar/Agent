...
class AgentCritic(AgentCriticPort):
    ...
    def critique(
        ...
        # Basic deterministic rules check first
        issues: list[str] = []

        # Rule 1: Hallucination of standard refusal or system error messages
        if "Internal Server Error" in response_text or "HTTP 500" in response_text:
            issues.append("Response contains server error text.")

        # Rule 2: If context exists but answer says 'no information' when context clearly has keywords
        ...
        # LLM-based evaluation if LLMProvider is provided
        if self.llm_provider is not None:
            try:
                ...
                req = LLMRequest(prompt=eval_prompt, max_tokens=100, temperature=0.0)
