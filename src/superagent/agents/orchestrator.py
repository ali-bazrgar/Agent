...
class AgentOrchestrator(AgentOrchestratorPort):
    ...
    def _multimodal_user_content(text: str, attachments: list[dict[str, Any]]) -> list[dict[str, Any]] | str:
        ...
        for attachment in attachments:
            ...
            if kind == "image": blocks.append({"type": "image_url", "image_url": {"url": f"data:{mime};base64,{data}"}})
                ...
                blocks.append({"type": "text", "text": f"[File: {name}]\n{text_content}" if isinstance(text_content, str) and text_content.strip() else f"[File attached: {name} ({mime})]. No text representation is available to the model for this file."})
        return blocks
    ...
    def execute(self, request: AgentRequest) -> AgentResponse:
        ...
        if runtime is not None:
            ...
            if runtime.max_output_tokens is not None: config.setdefault("max_tokens", runtime.max_output_tokens)
            config.setdefault("temperature", runtime.temperature)
            config.setdefault("top_p", runtime.top_p)
        # Direct orchestrator callers/tests may not inject a ModelRuntimeConfig.
        # Use neutral defaults rather than failing with a hidden KeyError.
        ...
        try:
            ...
            if plan.tool_required and not llm_driven_tools:
                ...
                if self.tool_executor is None: state.add_diagnostic("tool_error", "tool execution requested but no executor is configured")
                    ...
                    call = self._build_tool_call(request.message); state.reserve_tool_call(); result = self.tool_executor.execute_tool(call, ToolExecutionContext(execution_id=execution_id)); used_tools = True
            ...
            if plan.memory_required and self.memory_retriever is not None:
                try:
                    top_k = max(1, int(config.get("memory_recall_top_k", 5))); retrieved_memories = list(self.memory_retriever.retrieve_memories(request.message, top_k=top_k)); state.add_diagnostic("memory_recall", {"enabled": True, "top_k": top_k, "matches": len(retrieved_memories), "mode": "persistent_recall"})
            ...
            used_retrieval = bool(retrieved_chunks); used_memory = bool(retrieved_memories)
            ...
            context_window = int(config["context_window_tokens"]); reserved_output = int(config.get("reserved_output_tokens", 0))
            ...
            system_prompt = next((m["content"] for m in messages if m["role"] == "system"), None); user_prompt = next((m["content"] for m in reversed(messages) if m["role"] == "user"), request.message)
            ...
            if isinstance(attachments, list):
                ...
                messages[user_index]["content"] = self._multimodal_user_content(request.message, [a for a in attachments if isinstance(a, dict)]); state.add_diagnostic("attachment_count", len(attachments))

            iteration = 1; final_answer = ""; critique_res: CritiqueResult | None = None; verifier_res: VerificationResult | None = None; current_messages = messages
            provider_metadata = {**request.metadata, "_tool_call_reserver": state.reserve_tool_call}; tool_evidence_text: list[str] = []
            while iteration <= plan.max_iterations:
                ...
                if plan.critic_required:
                    state.transition_to(AgentExecutionStatus.CRITIQUING); state.reserve_model_call()
                ...
                if plan.verifier_required and (used_retrieval or used_tools):
                    state.transition_to(AgentExecutionStatus.VERIFYING); verifier_res = self.verifier.verify(request.message, final_answer, provenance)
                ...
                state.transition_to(AgentExecutionStatus.REVISING, {"iteration": iteration}); state.increment_retries(); iteration += 1
                feedback: list[str] = []
            ...
            if self.memory_lifecycle is not None:
                try: self.memory_lifecycle.process_turn(request, final_answer, execution_id=execution_id)
                except Exception as exc: state.add_diagnostic("memory_processing_error", str(exc)); logger.warning("Memory processing failed gracefully: %s", exc)
            state.transition_to(AgentExecutionStatus.COMPLETED)
            if self.execution_repository is not None: self.execution_repository.save_execution(execution_id, request.request_id, request.conversation_id, state.status.value, state.diagnostics)
    ...
    def _build_tool_call(self, message: str) -> ToolCall:
        ...
        if not match: raise ValueError("Unable to parse tool command")
