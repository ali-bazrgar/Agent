...
class AgentOrchestrator(AgentOrchestratorPort):
        ...
        for attachment in attachments:
            ...
            if kind == "image":
                ...
                if isinstance(text_content, str) and text_content.strip():
                    ...
                    blocks.append({"type": "text", "text": f"[File attached: {name} ({mime})]. This file is available to the client but has no text representation for the current model."})
        return blocks
    ...
    def execute(self, request: AgentRequest) -> AgentResponse:
        ...
        try:
            ...
            state.add_diagnostic("attachment_count", len(attachments) if isinstance(attachments, list) else 0)
            ...
            try:
                ...
                pass
            return AgentResponse(request_id=request.request_id, conversation_id=request.conversation_id, answer=f"Execution failed: {exc}", execution_id=execution_id, status=AgentExecutionStatus.FAILED, iterations=max(1, state.iterations), diagnostics=state.diagnostics)

    @staticmethod
    def _build_tool_call(message: str) -> ToolCall:
        match = re.search(r"(?:use|call|run)\s+(calculator|current_time|web_search|web_fetch)\s*(.*)$", message, flags=re.IGNORECASE)
        tool_name = match.group(1) if match else "calculator"
        raw_args = match.group(2).strip() if match else message
        arguments: dict[str, Any] = {"expression": raw_args} if tool_name == "calculator" else {"query": raw_args}
        return ToolCall(tool_call_id=f"tool-{uuid.uuid4().hex[:10]}", tool_name=tool_name, arguments=arguments)
