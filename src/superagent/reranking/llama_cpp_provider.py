...
class LlamaCppRerankerProvider(RerankerProvider):
    """HTTP provider for llama.cpp reranking."""
    ...
    def rerank(self, request: RerankRequest) -> RerankResponse:
        ...
        payload: dict[str, object] = {"query": request.query, "documents": request.candidates, "top_n": len(request.candidates)}
        if self.settings.reranker_model_id:
            payload["model"] = self.settings.reranker_model_id
        try:
        ...
        return RerankResponse(ranked_items=ranked_items, provider_name="llama.cpp")
        ...
        if isinstance(results, list):
            ...
            for index, item in enumerate(results):
                ...
                if parsed_score is None:
                    continue
                ranked_items.append({
                    "text": document_text if document_text is not None else candidates[index] if index < len(candidates) else "",
                    "score": parsed_score,
