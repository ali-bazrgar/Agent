...
class AppContainer:
    ...
    def __post_init__(self) -> None:
        ...
        if self.web_provider is None and self.settings.web_provider_base_url:
            self.web_provider = DefaultWebSearchProvider(search_url=self.settings.web_provider_base_url)

    @property
    def source_repository(self) -> SqliteSourceRepository:
        if self._source_repository is None:
            self._source_repository = SqliteSourceRepository(self.database_engine)
        return self._source_repository

    @property
    def document_repository(self) -> SqliteDocumentRepository:
        if self._document_repository is None:
            self._document_repository = SqliteDocumentRepository(self.database_engine)
        return self._document_repository

    @property
    def document_version_repository(self) -> SqliteDocumentVersionRepository:
        if self._document_version_repository is None:
            self._document_version_repository = SqliteDocumentVersionRepository(self.database_engine)
        return self._document_version_repository

    @property
    def chunk_repository(self) -> SqliteChunkRepository:
        if self._chunk_repository is None:
            self._chunk_repository = SqliteChunkRepository(self.database_engine)
    ...
    def embedding_repository(self) -> SqliteEmbeddingRepository:
        if self._embedding_repository is None:
            self._embedding_repository = SqliteEmbeddingRepository(self.database_engine)
    ...
    def execution_repository(self) -> SqliteExecutionRepository:
        if self._execution_repository is None:
            self._execution_repository = SqliteExecutionRepository(self.database_engine)
        return self._execution_repository

    @property
    def flashcard_repository(self) -> SqliteFlashcardRepository:
        if self._flashcard_repository is None:
            self._flashcard_repository = SqliteFlashcardRepository(self.database_engine)
        return self._flashcard_repository

    @property
    def review_repository(self) -> SqliteReviewRepository:
        if self._review_repository is None:
            self._review_repository = SqliteReviewRepository(self.database_engine)
    ...
    def agent_critic(self) -> AgentCritic:
        if self._agent_critic is None:
            self._agent_critic = AgentCritic(llm_provider=self.llm_provider)
        return self._agent_critic

    @property
    def agent_verifier(self) -> AgentVerifier:
        if self._agent_verifier is None:
            self._agent_verifier = AgentVerifier()
    ...
    def memory_retriever(self) -> DefaultMemoryRetriever:
        if self._memory_retriever is None:
            self._memory_retriever = DefaultMemoryRetriever(self.memory_repository)
