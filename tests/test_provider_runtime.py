...
def test_openai_compatible_provider_allows_request_level_sampling_overrides() -> None:
    captured: dict[str, object] = {}
    transport = httpx.MockTransport(lambda request: (captured.update(json.loads(request.content)) or httpx.Response(200, json={"choices": [{"message": {"content": "ok"}}]})))
    provider = OpenAICompatibleLLMProvider(_settings(), client=_client(transport))
    provider.complete(LLMRequest(prompt="hi", temperature=0.1, top_p=0.4, frequency_penalty=0.0, presence_penalty=-0.2, seed=7, max_tokens=99))

    assert captured["temperature"] == 0.1
    assert captured["top_p"] == 0.4
...
def test_provider_configuration_defaults_are_applied() -> None:
    ...
    assert settings.llm_frequency_penalty == 0.0
